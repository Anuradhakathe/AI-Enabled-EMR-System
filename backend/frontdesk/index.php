...
if (isset($_POST['doc_login'])) {
    ...
    $username = $_POST['doc_number']; // Used as ID number
    $password = sha1(md5($_POST['doc_pwd'])); // Double encryption
    ...
    if ($user_type == "Doctor") {
        ...
        // ✅ Updated to login using fd_number
        $stmt = $mysqli->prepare("SELECT ad_id, fd_number FROM his_admin WHERE fd_number=? AND ad_pwd=? AND role='frontdesk'");
        $stmt->bind_param('ss', $username, $password);
        $stmt->execute();
        $stmt->bind_result($fd_id, $fd_number);
        if ($stmt->fetch()) {
            $_SESSION['frontdesk_id'] = $fd_id;
            $_SESSION['frontdesk_number'] = $fd_number;
