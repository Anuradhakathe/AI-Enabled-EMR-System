...
if (isset($_POST['doc_login'])) {
    ...
    if ($user_type == "Doctor") {
        ...
        $stmt = $mysqli->prepare("SELECT ad_id, ad_fname FROM his_admin WHERE fd_number = ? AND ad_pwd = ? AND role='frontdesk'");
        if (!$stmt) {
            die("Prepare failed: " . $mysqli->error);
        }
        $stmt->bind_param('ss', $username, $password);
        $stmt->execute();
        $stmt->bind_result($fd_id, $fd_fname);
    
        if ($stmt->fetch()) {
            // Use frontdesk-specific session variable
            $_SESSION['fd_id'] = $fd_id;
            $_SESSION['fd_name'] = $fd_fname;
